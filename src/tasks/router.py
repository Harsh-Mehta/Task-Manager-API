...
from src.tasks.crud import create_task, delete_task, get_all_tasks, get_task, update_task
...
@router.get("/tasks/{task_id}", response_model=TaskSingleResponse, tags=["tasks"])
def get_task_by_id(task_id: int, db: Session = Depends(get_db)):
    ...
    if not task:
        raise HTTPException(status_code=404, detail="No task not found with ID provided")

    return {"status_code": 200, "task": task}


@router.post("/tasks", response_model=TaskInDB, tags=["tasks"], )
def create_new_task(details: TaskCreate, db: Session = Depends(get_db)):
    ...
    **Errors:**
    ...
    if invalid:
        raise HTTPException(status_code=400, detail="Title cannot be empty or only contain whitespace(s)")
...
@router.put("/tasks/{task_id}", response_model=TaskInDB, tags=["tasks"])
def modify_task(task_id: int, details: TaskUpdate, db: Session = Depends(get_db)):
    """ This endpoint updates an existing task.
    If the request body is invalid or the task has not been created, a 400 error is returned. """
...
@router.patch("/tasks/{task_id}", response_model=TaskInDB, tags=["tasks"])
def partial_update_task(
    task_id: int, details: TaskPartialUpdate, db: Session = Depends(get_db)
):
    """ This endpoint partially updates the details an existing task. If the request body is invalid or the task has not been created, a 400 error is returned. """
...
@router.delete("/tasks/{task_id}", response_model=BaseResponse, tags=["tasks"])
def remove_task(task_id: int, db: Session = Depends(get_db)):
    ...
    if not task:
        raise HTTPException(status_code=500, detail="Unable to delete the task")
